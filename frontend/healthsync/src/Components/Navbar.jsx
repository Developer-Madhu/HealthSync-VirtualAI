...
function Navbar() {
  ...
  const userType = localStorage.getItem("userType"); // Assuming userType is stored as 'doctor' or 'patient'
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between h-16">
          ...
          <div className="flex space-x-8">
            ...
            {token ? (
              <Link to={dashboardLink} className="inline-flex items-center px-1 pt-1 text-gray-500 hover:text-indigo-600 transition-colors">
                Dashboard
              </Link>
            ) : (
              <Link to="/signup" className="inline-flex items-center px-1 pt-1 text-gray-500 hover:text-indigo-600 transition-colors">
